...
- s3fs-backed paths are transparently redirected to their local on-disk cache copy: DataSource::open runs every path through the new DataSource::localCachePath, which parses fstab for s3fs#... mounts and their use_cache= directory and, when the requested file lives under such a mount and a cached copy exists, opens <use_cache>[/<bucket>]/<rel> instead so the file is memory-mapped from local disk rather than round-tripped through the fuse layer. No-op off s3fs or for a not-yet-cached object (the fuse read then populates the cache for next time); directory listing still goes through the mount.
